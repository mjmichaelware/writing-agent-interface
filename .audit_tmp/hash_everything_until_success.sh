...
for i in $(seq 1 "$ATTEMPTS"); do
  echo
  echo "===== FULL HASH ATTEMPT $i / $ATTEMPTS ====="
  ...
  if [ "$STATUS" = "success" ]; then
    echo
    echo "===== HASH FINISHED SUCCESSFULLY ====="
  ...
  gh run view "$RUN_ID" --log-failed | tail -n 120 || true

  if [ "$i" -lt "$ATTEMPTS" ]; then
    echo
    echo "Sleeping $SLEEP_SECONDS seconds before next full retry..."
    sleep "$SLEEP_SECONDS"
  fi
done

echo
echo "===== NO SUCCESS AFTER ALL ATTEMPTS ====="
exit 1
