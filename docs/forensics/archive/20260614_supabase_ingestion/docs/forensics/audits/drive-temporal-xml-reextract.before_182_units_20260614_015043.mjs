...
async function main() {
  ...
  for (const pair of targetPairs) {
    if (!uniqueDocs.has(pair.drive_doc.id)) uniqueDocs.set(pair.drive_doc.id, { doc: pair.drive_doc, pairs: [] });
    uniqueDocs.get(pair.drive_doc.id).pairs.push(pair);
  }
  ...
  const extractionByDriveId = {};

  const uniqueEntries = [...uniqueDocs.values()];
  for (let i = 0; i < uniqueEntries.length; i++) {
    const { doc, pairs } = uniqueEntries[i];

    const docDir = path.join(DEST, driveDocRelDir(doc));
    mkdirp(docDir);

    safeWriteJson(path.join(docDir, "drive-file-metadata.json"), doc);
    safeWriteJson(path.join(docDir, "incoming-import-targets.json"), {
      drive_id: doc.id,
      drive_name: doc.name,
      import_target_count: pairs.length,
      imports: pairs.map((p) => ({
        target_index: p.target_index,
        local_path: p.local_path,
        local_basename: p.local_basename,
    ...
    const current = await exportCurrentDocx(doc, docDir, token);
    if (current.document_xml) currentXmlCount++;

    const docsApi = await saveDocsApi(doc, docDir, token);
    const comments = await saveComments(doc, docDir, token);
    const revisions = await saveV2Revisions(doc, docDir, token);
    ...
    extractionByDriveId[doc.id] = extractionSummary;

    for (const pair of pairs) {
      writeImportManifest(DEST, pair, extractionSummary);
      importManifestCount++;
    }
    ...
    summaryRows.push([
      ...
      doc.name.replace(/	/g, " "),
      pairs.length,
      pairs.map((p) => p.target_index).join(","),
      pairs.map((p) => p.local_basename.replace(/	/g, " ")).join(" | "),
      pairs.map((p) => p.match_score).join(","),
      current.bytes || 0,
    ...
    console.error(
      `progress=${i + 1}/${uniqueEntries.length} imports=${importManifestCount} current_xml=${currentXmlCount} revision_xml=${revXmlTotal} drive="${doc.name}"`
    );
    await sleep(SLEEP_MS);
  }

  safeWriteJson(path.join(DEST, "extraction-by-drive-id.json"), extractionByDriveId);
  safeWriteText(path.join(DEST, "drive-temporal-summary.tsv"), summaryRows.join("\n") + "\n");
  ...
  const totals = {
    ...
    unique_drive_docs_extracted: uniqueEntries.length,
  ...
  console.log(`unique_drive_docs_extracted=${totals.unique_drive_docs_extracted}`);
