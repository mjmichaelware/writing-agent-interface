 ...
 * Retractable Cinema of Functions floating above the manuscript.
 */
  ...
  return (
    <AnimatePresence>
      {isOpen && (
        ...
        >
          <div className="absolute inset-0 bg-black/90 backdrop-blur-3xl" onClick={close} />
          <div className="relative w-full max-w-xl h-full bg-zinc-950 border-l border-cyan-900/30 p-12 space-y-12 shadow-2xl overflow-y-auto">
            ...
            <div className="grid grid-cols-1 gap-8">
              <div className="p-8 bg-zinc-900/50 border border-zinc-800 space-y-6">
                <p className="text-[10px] text-zinc-500 uppercase tracking-widest font-sans">Graph Manifest</p>
                <p className="text-3xl font-light text-cyan-400 tracking-tighter">{nodeCount} Nodes Active</p>
                <button className="w-full bg-cyan-950/40 p-4 text-left text-[10px] tracking-widest uppercase hover:bg-cyan-900 transition-all border border-cyan-900/20">Search 181 Nodes</button>
              </div>
              <div className="space-y-4">
                <p className="text-[10px] text-zinc-500 uppercase tracking-widest font-sans">EMA Functions</p>
                <button className="w-full bg-zinc-900 p-4 text-left text-[10px] tracking-widest uppercase hover:bg-cyan-900 transition-all">Biblical Hyperlinks</button>
                <button className="w-full bg-zinc-900 p-4 text-left text-[10px] tracking-widest uppercase hover:bg-cyan-900 transition-all">Narrative Foreshadowing</button>
                <button className="w-full bg-zinc-900 p-4 text-left text-[10px] tracking-widest uppercase hover:bg-cyan-900 transition-all">EMA Metadata Viewer</button>
              </div>
            </div>
          </div>
        </motion.div>
...
export default function SingularityOS() {
    ...
    fetch('/api/chapters?slug=7').then(r => r.json()).then(d => {
      if (d.xml) {
        ...
        setParagraphs(Array.from(xmlDoc.getElementsByTagName("paragraph")).map((p, i) => ({
          ...
          isBlood: i > 12 // Paragraph 13 viscerality shift
        })));
  ...
  return (
    <div ref={containerRef} className="relative bg-[#000000] text-white min-h-[1000vh] font-[var(--font-hebrew)] selection:bg-cyan-500/30">
      ...
      {/* LAYER 3: HUD (Z-50) */}
      <button onClick={() => setIsDashOpen(true)} className="fixed top-8 right-8 z-40 bg-zinc-900/40 p-4 border border-zinc-800 text-[10px] tracking-widest uppercase hover:bg-cyan-900 transition-all backdrop-blur-md">
        Dashboard
      </button>
      ...
      <div className="relative z-20">
        ...
        <section className="py-80 text-center bg-black/60 border-t border-zinc-900/50">
          <h3 className="text-white uppercase tracking-[0.6em] text-[10px] mb-10 font-sans opacity-40 uppercase tracking-widest">Architect of the Singularity</h3>
