...
export const OmniText = ({ content }: { content: string }) => {
  ...
  const tokens = useMemo(() => tokenize(content).map(t => [t, t.normalize('NFC').toLocaleLowerCase()] as const), [content]);
  let counter = 0;
  return (
    <div className="text-2xl md:text-3xl leading-[2.1] font-serif">
      {tokens.map(([t, n], i) => (
        <Word key={i} index={isWordToken(t) ? counter++ : -1} text={t} isSelected={activeLower === n} />
      ))}
    </div>
  );
};
